...
def create_test_table(file_name):
    ...
    data = [
        ...
        [r"П?че\dу", "поче5у", "=SEARCH(A12,B12)"],
...
class TestSearchCcToken(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        create_test_table("tests/search.xlsx")
        cls.parser = (
            Parser()
            .set_excel_file_path("tests/search.xlsx")
            .enable_safety_check()
    ...
    def test_with_asterisk(self):
        ...
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 2))
    ...
    def test_not_found_three_args(self):
        excepted_cell_value = "#VALUE!"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 5))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_reg(self):
        excepted_cell_value = "#VALUE!"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 6))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_asterisk_and_question(self):
        excepted_cell_value = 5
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 7))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_with_tilda(self):
        excepted_cell_value = 9
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 8))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_asterisk_empty(self):
        excepted_cell_value = 1
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 9))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_question_empty(self):
        excepted_cell_value = "#VALUE!"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 10))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_reg2(self):
        excepted_cell_value = "#VALUE!"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 11))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_start_with_question(self):
        excepted_cell_value = 1
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 12))
        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_start_with_asterisk(self):
        excepted_cell_value = 1
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 2, 13))
        self.assertEqual(cell_value.value, excepted_cell_value)
