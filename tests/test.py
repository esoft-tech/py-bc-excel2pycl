...
class TestTokens(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        create_test_table("tests/tokens.xlsx")
        cls.parser = (
            Parser()
    ...
    def test_column_matrix_arg_token(self):
        ...
        self.assertEqual(
            [cell.value for cell in expected],
            [cell.value for cell in actual],
            msg="Column token matrix arg down",
        )
    ...
    def test_index_array_return(self):
        ...
        cell_values_array = [
            [
                cell.value
                for cell in self.executor.get_cells([Cell("index", 0, 0), Cell("index", 1, 0), Cell("index", 2, 0)])
            ],
            [
                cell.value
                for cell in self.executor.get_cells([Cell("index", 0, 1), Cell("index", 1, 1), Cell("index", 2, 1)])
            ],
            [
                cell.value
                for cell in self.executor.get_cells([Cell("index", 0, 2), Cell("index", 1, 2), Cell("index", 2, 2)])
            ],
        ]
