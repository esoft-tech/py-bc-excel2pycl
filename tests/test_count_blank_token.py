...
class TestCountBlankCcToken(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        create_test_table("tests/count_blank.xlsx")
        cls.parser = (
            Parser()
            .set_excel_file_path("tests/count_blank.xlsx")
            .enable_safety_check()
    ...
    def tearDownClass(cls) -> None:
        ...
        os.remove("tests/count_blank.xlsx")

    def test_standard(self):
        excepted_cell_value = 3
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 5, 0))

        self.assertEqual(cell_value.value, excepted_cell_value, msg="test standart")

    def test_cell_concat(self):
        excepted_cell_value = 3
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 5, 1))

        self.assertEqual(cell_value.value, excepted_cell_value, msg="test sell concat")

    def test_rectangle(self):
        excepted_cell_value = 6
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 5, 2))
