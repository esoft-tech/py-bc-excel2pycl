...
class TestIfsToken(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        create_test_table("tests/ifs.xlsx")
        cls.parser = (
            Parser()
            .set_excel_file_path("tests/ifs.xlsx")
            .enable_safety_check()
    ...
    def tearDownClass(cls) -> None:
        ...
        os.remove("tests/ifs.xlsx")

    def test_true_in_first(self):
        excepted_cell_value = "A"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 0))
    ...
    def test_true_in_last(self):
        ...
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 3))
    ...
    def test_true_in_middle(self):
        ...
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 2))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_not_true(self):
        excepted_cell_value = "#N/A"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 4))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_compare_word_and_number(self):
        excepted_cell_value = "меньше"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 5))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_compare_str_as_number(self):
        excepted_cell_value = "верно"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 7))

        self.assertEqual(cell_value.value, excepted_cell_value)

    def test_compare_sell_and_sell(self):
        excepted_cell_value = "B1<=B2"
        cell_value = Executor().set_executed_class(class_file=self.translation_file_path).get_cell(Cell(0, 1, 8))
