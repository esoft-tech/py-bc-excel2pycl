...
class VlookupControlConstructionTokenTranslator(AbstractTranslator[VlookupControlConstructionToken]):
    @classmethod
    def translate(cls, token: VlookupControlConstructionToken, excel: Excel, context: Context) -> str:
        ...
        return context.set_sub_cell(
            token.in_cell,
            f"self._vlookup({lookup_value}, {matrix}, {column_number}, {range_lookup})",
        )
