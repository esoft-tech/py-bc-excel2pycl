...
class SumIfControlConstructionTokenTranslator(AbstractTranslator[SumIfControlConstructionToken]):
    @classmethod
    def translate(cls, token: SumIfControlConstructionToken, excel: Excel, context: Context) -> str:
        if token.cell:
            ...
            raise E2PyclTranslationException("Invalid cells range", token, context)
        ...
        sum_range = MatrixOfCellIdentifiersTokenTranslator.translate_with_cells(
            start_cell_of_needed,
            finish_cell_of_needed,
            excel,
            context,
        )
