...
class IterableRangeOfCellIdentifierWithConditionTokenTranslator(AbstractTranslator):
    @classmethod
    def translate(cls, token: IterableRangeOfCellIdentifierWithConditionToken, excel: Excel, context: Context) -> str:
        ...
        return context.set_sub_cell(
            token.in_cell, '[' + ', '.join(
                [f'*{RangeOfCellIdentifierWithConditionTokenTranslator.translate(i, excel, context)}' for i in
                    token.range_of_cell_identifiers_with_conditions]
                ) + ']'
            )
