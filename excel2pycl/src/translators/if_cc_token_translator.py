...
class IfControlConstructionTokenTranslator(AbstractTranslator):
    ...
    def translate(cls, token: IfControlConstructionToken, excel: Excel, context: Context) -> str:
        ...
        condition, when_true, when_false = ExpressionTokenTranslator.translate(
            token.condition, excel, context
        ), ExpressionTokenTranslator.translate(
            token.when_true, excel, context
        ), ExpressionTokenTranslator.translate(token.when_false, excel, context) if token.when_false else 'False'
